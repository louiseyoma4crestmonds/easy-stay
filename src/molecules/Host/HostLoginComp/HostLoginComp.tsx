...
import styles from "./HostLoginComp.module.css";

function HostLoginComp() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      //   signinUser();
      router.push({ pathname: "/host/dashboard" });
    } catch (err: any) {
  ...
  const handleForgotPasswordClick = () => {
    ...
    router.push("/host/recover-password");
  };
  ...
  return (
    <div className={styles.maindiv}>
      ...
      <form className={styles.formDiv} onSubmit={handleLogin}>
        <p className={styles.loginP1}>Log in as a Host</p>
        ...
        <div className={styles.formButtonDiv}>
          ...
          <Button
            ...
            imageWidth={24}
            height={24}
            onClick={() => signIn("google", { callbackUrl: "/host/signin" })}
          >
            Sign in with Google
          </Button>
        </div>
